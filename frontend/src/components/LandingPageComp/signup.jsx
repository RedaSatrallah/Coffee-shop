...
export default function Signup() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!validate()) return;//Si la validation échoue, on arrête l’exécution de la fonction.
        axios.post('http://localhost:5000/Signup', { name, email, password, confirmPassword })
            .then((result) => {
        ...
        <div className="min-h-screen bg-[#FFF3EB] text-[#3B170D] font-sans overflow-hidden">
            ...
            <main className="max-w-[1180px] mx-auto mt-3.5 mb-10 px-6 py-6 bg-[#3B170D]/10  rounded-[5px]">
                <section className="grid grid-cols-[1.1fr_.9fr] gap-10 items-stretch max-[980px]:grid-cols-1">
                    ...
                    <div className="flex flex-col gap-5 justify-center h-[700px]  py-1.5 ">
                        <h1 className="font-display font-instrument text-center text-[54px] leading-[1.05] tracking-[.4px] mb-6 max-[980px]:text-[44px]">Create Account</h1>
                        <form className="flex flex-col gap-5" onSubmit={handleSubmit} >
                            <input className={`w-full h-12 border border-[#3B170D] rounded-[5px] px-3.5 bg-[#FFF3EB] outline-none  placeholder:text-[#3B170D]/60 focus:border-[#3B170D] focus:ring-4 focus:ring-[#3B170D]/30 ${errors.name ? "border-red-500" : "border-[#3B170D]"
                                    }`} placeholder="Username" name="username" onChange={(e) => setName(e.target.value)} />{/*Chaque fois que l’utilisateur écrit, on récupère le texte et on le stocke dans une variable React. */}
                            {errors.name && (
                            ...
                            <input
                                ...
                                className={`w-full h-12 border border-[#3B170D] rounded-[5px] px-3.5 bg-[#FFF3EB] outline-none  placeholder:text-[#3B170D]/60 focus:border-[#3B170D] focus:ring-4 focus:ring-[#3B170D]/30 ${errors.confirmPassword ? "border-red-500" : "border-[#3B170D]"
                                    }`}
                                onChange={(e) => setConfirmPassword(e.target.value)}
                            />

                             {errors.confirmPassword && (//condition && valeur
                            ...
                            <div className="text-center text-[18px] text-[#3B170D]/80">
                                ...
                                <a href="/Login" className="text-[#3B170D] font-instrument no-underline hover:underline">
                                    Log in
                                </a>
                            </div>
                        </form>
